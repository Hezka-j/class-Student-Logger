...
public:
	static void DoHardWork()
	{
		...
		HardProcessor processor(1); // начало диапазона
...
public:
	~Student() {
		...
		SYSTEMTIME st;
		GetSystemTime(&st);
		Logger::GetInstance()->Write(to_string(st.wHour + 2) + ":" + to_string(st.wMinute) + ":" + to_string(st.wMilliseconds / 1000.0) + "  " + to_string(st.wDay) + "/" + to_string(st.wMonth) + "/" + to_string(st.wYear));
		Logger::GetInstance()->Write(" Student died\n");
	}
	...
	Student(string name, string surname, string number)
	{
		...
		GetSystemTime(&st);
		Logger::GetInstance()->Write(to_string(st.wHour + 2) + ":" + to_string(st.wMinute) + ":" + to_string(st.wMilliseconds / 1000.0) + "  " + to_string(st.wDay) + "/" + to_string(st.wMonth) + "/" + to_string(st.wYear));
		Logger::GetInstance()->Write(" Student has been born\n");
